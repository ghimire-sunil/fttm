...
class purchaseOrder(models.Model):
    ...
    def action_open_file_upload_wizard(self):
        ...
        return {
            ...
            'res_model': 'purchase.order.file.upload.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
...
class purchaseOrderFileUploadWizard(models.TransientModel):
    _name = 'purchase.order.file.upload.wizard'
    _description = 'purchase Order File Upload Wizard'
    ...
    def action_upload_file(self):
        ...
        result = self._import_purchase_orders(rows)
    ...
    def _import_purchase_orders(self, rows):
        created_count = 0
        updated_count = 0
        orders_data = {}
        row_number = 1
        ...
        for row in rows:
            ...
            if self._is_empty_value(order_ref):
                if last_order_ref:
                    ...
                    order_ref = self.env['ir.sequence'].next_by_code('purchase.order') or '/'
        ...
        for order_ref in orders_data:
            ...
            try:
                self._create_purchase_order(order_ref, order_data)
                created_count += 1
            except Exception as e:
    ...
    def _create_purchase_order(self, order_ref, order_data):
        ...
        for line_data in order_data['lines']:
            self._create_order_line(purchase_order, line_data)
        
        return purchase_order

    def _create_order_line(self, purchase_order, line_data):
        barcode_value = line_data.get('Order Lines/Product/Barcode')
        
        if self._is_empty_value(barcode_value):
            raise UserError(f'Product barcode is required for order {purchase_order.name}')
        
        barcode = str(barcode_value).strip()
        
        product = self.env['product.product'].search([('barcode', '=', barcode)], limit=1)
        if not product:
            raise UserError('Product with barcode "' + barcode + '" not found in order ' + purchase_order.name)
        ...
        if uom_value and not self._is_empty_value(uom_value):
            ...
            if uom:
                ...
                _logger.warning(f'Unit of Measure "{uom_name}" not found for order {purchase_order.name}, using product default')
            ...
            for tax_name in tax_name_list:
                if tax_name:
                    ...
                    if tax:
                        ...
                        _logger.warning(f'Tax "{tax_name}" not found for company {purchase_order.company_id.name} in order {purchase_order.name}')
